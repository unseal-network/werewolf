  ...
  private async runCurrentSpeaker(
    ...
    const voiceAgent =
      this.voiceAgents && player.kind === "agent"
        ...
        : null;
    let emittedTtsProgress = false;
    const deltaStream = hasValidAgentSpeech && !voiceAgent
      ...
      hasValidAgentSpeech && voiceAgent
        ? Promise.resolve(
            voiceAgent.speak(
              speech,
              playerId,
              room.timing.agentSpeechRate ?? 1,
              (progressText) => {
                emittedTtsProgress = true;
                this.emitAgentSpeechProgress(
                  room,
                  playerId,
                  progressText,
                  expectedSpeechTurn
