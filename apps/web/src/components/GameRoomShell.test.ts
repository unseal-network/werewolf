...
describe("game room seat layout", () => {
  ...
  it("requests a publish-capable LiveKit token only when voice recording starts", () => {
    ...
    expect(gameRoute).toContain("const [livekitCanPublish, setLivekitCanPublish]");
    expect(gameRoute).toContain("const requestPublishVoiceToken = useCallback");
    expect(gameRoute).toContain("if (!myPlayerId)");
    expect(gameRoute).toContain("canPublishVoice={livekitCanPublish}");
    expect(gameRoute).toContain("onRequestPublishVoice={requestPublishVoiceToken}");
    expect(voicePanel).toContain("if (!canPublishVoice && onRequestPublishVoice)");
    expect(voicePanel).toContain("pendingMicStartRef.current = true");
  });
  ...
  it("keeps LiveKit credential refresh independent from the SSE room snapshot", () => {
    const gameRoute = readFileSync(
      resolve(process.cwd(), "apps/web/src/routes/game.$gameRoomId.tsx"),
      "utf8"
    );
    const voiceEffectStart = gameRoute.indexOf("Fetch LiveKit credentials independently");
    const voiceEffectEnd = gameRoute.indexOf("const requestPublishVoiceToken", voiceEffectStart);
    const voiceEffect = gameRoute.slice(voiceEffectStart, voiceEffectEnd);

    expect(voiceEffect).toContain("livekitCredentialKeyRef");
    expect(voiceEffect).toContain("if (!matrixUserId)");
    expect(voiceEffect).toContain("getLivekitToken(gameRoomId)");
    expect(voiceEffect).toContain("livekitToken");
    expect(voiceEffect).toContain("livekitServerUrl");
    expect(voiceEffect).not.toContain("roomIdForVoice");
    expect(voiceEffect).not.toContain("voiceIdentityMode");
    expect(voiceEffect).not.toContain("!myPlayerId");
    expect(voiceEffect).not.toContain("!room");
    expect(gameRoute).not.toContain("setVoiceTokenRefreshNonce");
  });
