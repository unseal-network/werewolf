...
describe("radial player picker", () => {
  ...
  it("keeps the old visual-runtime radial picker overrides removed", () => {
    const css = readFileSync(resolve(process.cwd(), "apps/web/src/styles/game-room/legacy.css"), "utf8");

    expect(css).not.toContain("visual-runtime-root");
  });
});
